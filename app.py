...
# ============================================
# 🔥 CRITICAL FIX: DATABASE SETUP FUNCTION 🔥
# ============================================
def setup_database():
    ...
    try:
        ...
        # Verify download
        if db_file.exists() and db_file.stat().st_size > 0:
            logger.info(f"✅ Database downloaded successfully! Size: {db_file.stat().st_size} bytes")
            return str(db_file)
        else:
        ...
        try:
            st.warning("Trying alternative download source...")
            alt_url = "https://www.sqlitetutorial.net/wp-content/uploads/2018/03/chinook.zip"
            
            # Download zip
            zip_path = data_dir / "chinook.zip"
            ...
            if db_file.exists() and db_file.stat().st_size > 0:
                logger.info(f"✅ Database downloaded from alternative source! Size: {db_file.stat().st_size} bytes")
                return str(db_file)
                
        except Exception as e2:
            logger.error(f"❌ Alternative download failed: {str(e2)}")
            st.error("❌ Failed to download database. Please check your internet connection.")
            return None
...
def initialize_analyst():
    ...
    try:
        ...
        # ✅ Setup database using the function
        db_path = setup_database()
        
        # ✅ Check if database was set up successfully
        if db_path is None:
